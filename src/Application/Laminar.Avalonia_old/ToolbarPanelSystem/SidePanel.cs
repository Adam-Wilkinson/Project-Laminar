...
internal class SidePanel
{
    ...
    public bool TryPlaceControl(ToolbarManager grip, Point p)
    {
        ...
        foreach (DragRecepticle recepticle in toolbarDragHitBoxes)
        {
            if (recepticle.HitBox.Contains(p))
            {
                ...
                {
                    grip.DockDirection = GetDockDirection();
                    return true;
                }
    ...
    private DockDirection GetDockDirection() => (Orientation, reverse) switch
    {
        (Orientation.Horizontal, false) => DockDirection.Top,
        (Orientation.Horizontal, true) => DockDirection.Bottom,
        (Orientation.Vertical, false) => DockDirection.Left,
        (Orientation.Vertical, true) => DockDirection.Right,
        _ => DockDirection.Top,
    };
}
