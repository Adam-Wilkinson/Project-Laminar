...
public sealed class CompositeDisposable : ICollection<IDisposable>
{
    ...
    /// </summary>
    private sealed class CompositeEnumerator(IDisposable?[] disposables) : IEnumerator<IDisposable>
    {
        private int _index = -1;

        public IDisposable Current => disposables[_index]!; // NB: _index is only advanced to non-null positions.

        object IEnumerator.Current => disposables[_index]!;
        ...
        public void Dispose()
        {
            ...
            var disposables1 = disposables;
            Array.Clear(disposables1, 0, disposables1.Length);
        }
        ...
        public bool MoveNext()
        {
            ...
            for (; ; )
            {
                ...
                if (idx >= disposables1.Length)
                {
                    return false;
                }

                // inlined that filter for null elements
                if (disposables1[idx] != null)
                {
                    return true;
                }
            }
