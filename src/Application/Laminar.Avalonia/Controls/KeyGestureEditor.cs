...
public class KeyGestureEditor : TemplatedControl
{
    ...
    {
        this.GetObservable(GestureProperty).Subscribe(new Domain.Observables.Value.AnonymousObserver<KeyGesture>(newValue =>
        {
            _currentGesture = newValue;
            RefreshText();
        }));

        this.GetObservable(FormatStringProperty).Subscribe(new Domain.Observables.Value.AnonymousObserver<string?>(newFormatString =>
        {
            RefreshText();
        }));

        this.GetObservable(StateProperty).Subscribe(new Domain.Observables.Value.AnonymousObserver<KeyGestureInterfaceState>(newValue =>
        {
