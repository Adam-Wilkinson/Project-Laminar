...
{    
    ...
    public void OnApplicationBuilt()
    {
        topLevel.GetResourceObservable(QuickAccessExtension.QuickAccessRepositoryKey)
            .Subscribe(new Domain.Observables.Value.AnonymousObserver<object?>(x =>
        {
        ...
        topLevel.GetResourceObservable(Tool.ToolRootKey).Subscribe(new Domain.Observables.Value.AnonymousObserver<object?>(x =>
        {
