...
public class CollectionSerializerFactory(ISerializer serializer) : IConditionalSerializerFactory
{
    public IConditionalSerializer? TryCreateSerializerFor(Type typeToSerialize)
    {
        if (typeToSerialize.GetInterfaces().FirstOrDefault(
        ...
        if (typeToSerialize.GetConstructor([]) is not null
            ...
            && addMethod.GetParameters()[0].ParameterType == enumerableType.GetGenericArguments()[0])
        {
            return Activator.CreateInstance(
                typeof(CollectionSerializer<,,>).MakeGenericType(
...
public class CollectionSerializer<TElement, TSerialized, TEnumerable>(ISerializer serializer) 
    : TypeSerializer<TEnumerable, SerializedCollection<TSerialized, TEnumerable>>
...
public class SerializedCollection<TSerializedElement, TEnumerable>(IEnumerable<TSerializedElement> serialized)
    : ICollection<TSerializedElement>
{
    private IEnumerable<TSerializedElement> _enumerable = serialized;

    public SerializedCollection() : this(new List<TSerializedElement>())
    {
    }
    ...
    {
        if (_enumerable is not List<TSerializedElement>)
        {
